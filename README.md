...
| モード | 内容 | こんな 時に |
| --- | --- | --- |
| 🫧 クラシック | 難度上昇 + コンボで ハイスコアを 狙う | しっかり 集中したい |
| 🌿 禅 | ゆったり 無限プレイ、コンボなし、揺れも控えめ | 仕事の 合間に 頭をリセット |
| 💢 仕事ストレス爆破 | `会議` `締切` `メール` `上司` などの **ストレスワード入りバブル** を 粉砕 | 残業前 / 月曜の朝 |
...
> 仕事ストレス爆破モードでは `粉砕!` `撃破!` `スカッ!` が 飛びます。
...
- `R` キー : 同じ モードで リスタート
- `Esc` キー : メニューに 戻る
- 右上の ☰ : メニューに 戻る  /  ⚙ : 設定 ( サウンド / 振動 / エフェクト量 / 難易度 )
